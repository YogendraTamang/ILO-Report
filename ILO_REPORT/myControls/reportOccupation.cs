...
{
    public partial class ucReportOccupation_ga : UserControl
    {
        businessClass bl = new businessClass();
        private int pTypeID;
        public ucReportOccupation_ga(int fldTypeID)
        {
            InitializeComponent();
            pTypeID = fldTypeID;
        }
        ...
        private void ucOccupation_ga_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        {
            string condition = "fldTypeID =" + fldTypeID;
            DataSet ds = bl.GetOccupation(condition);
        ...
        {
            string condition = "tblMain.fldTypeID=" + pTypeID.ToString();

            int sPlace = comboMunicipality.SelectedIndex;
            if (sPlace == 0)
                condition += " and tblMain.fldLocation=N'पनौती'";
            else if (sPlace == 1)
                condition += " and tblMain.fldLocation=N'धुलिखेल'";
            DataSet ds = bl.GetOccupation(condition);
            //DataTable dtbl = new DataTable();
            //dtbl.Columns.Add("fldLabourCause");
            if (ds != null && ds.Tables.Count > 0)
