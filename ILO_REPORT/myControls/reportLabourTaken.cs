...
{
    public partial class ucReportLabourTaken_Ka : UserControl
    {
        businessClass bl = new businessClass();
        private int pTypeID;
        public ucReportLabourTaken_Ka(int fldTypeID)
        {
            InitializeComponent();
            pTypeID = fldTypeID;
        }
        ...
        private void ucLabourTaken_ka_Load(object sender, EventArgs e)
        {
            try
            {
                loadData(pTypeID);
        ...
        {
            DataSet ds = bl.GetLabourTakenReport(fldTypeID);
